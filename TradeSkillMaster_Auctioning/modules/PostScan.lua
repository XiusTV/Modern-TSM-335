...
local bagInfoNeedsUpdate = true  -- OPTIMIZED: Track if bag update is needed
local bagLocationMap = {}  -- OPTIMIZED: Hash map for O(1) item location lookup
...
-- OPTIMIZED: Only update bag state when actually needed (event-driven)
function Post:UpdateBagState()
	if not bagInfoNeedsUpdate and time() == bagInfoUpdate then return end
	wipe(bagInfo)
	wipe(bagState)
	wipe(bagLocationMap)  -- OPTIMIZED: Clear location map
	...
	for bag, slot, itemString, quantity in TSMAPI:GetBagIterator(true) do
		...
		-- OPTIMIZED: Build location hash map for O(1) lookup
		if not bagLocationMap[itemString] then
			bagLocationMap[itemString] = {}
		end
		tinsert(bagLocationMap[itemString], {bag=bag, slot=slot, quantity=quantity})
	end
	
	bagInfoUpdate = time()
	bagInfoNeedsUpdate = false
end

-- OPTIMIZED: Mark bag state as dirty on bag updates
function Post:OnBagUpdate()
	bagInfoNeedsUpdate = true
end
...
function Post:FindItemSlot(findItemString, allLocations)
	...
	local locations = bagLocationMap[findItemString]
	if not locations or #locations == 0 then
		return allLocations and {} or nil, nil
	end
	
	if not allLocations then
		-- Return first location
		return locations[1].bag, locations[1].slot
	else
		-- Return all locations (shallow copy to preserve original)
		local result = {}

...
-- Default TSM design settings (Modern Dark Theme by default)
TSM.designDefaults = {
	frameColors = {
		-- Main frame background (modern dark)
		frameBG = { 
...
function TSM:RegisterModule()
	...
	TSM.slashCommands = {
		...
		{ key = "theme", label = "Changes the TSM visual theme (modern/classic/highcontrast)", callback = function(themeKey)
			if not themeKey or themeKey == "" then
				TSM:Print("|cff00fe00Available TSM Themes:|r")
				TSM:Print("  |cffFFD100/tsm theme modern|r - Modern Dark (sleek retail-style)")
				TSM:Print("  |cffFFD100/tsm theme classic|r - Classic TSM (original look)")
				TSM:Print("  |cffFFD100/tsm theme highcontrast|r - High Contrast (maximum visibility)")
				local current = TSM:GetCurrentTheme() or "modern"
				TSM:Print("Current theme: |cffFFD100" .. current .. "|r")
			else

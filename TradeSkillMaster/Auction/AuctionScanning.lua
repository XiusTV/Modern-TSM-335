...
-- OPTIMIZED: Hash-based duplicate detection - much faster than field-by-field comparison
local function GetPageHash()
	local shown = GetNumAuctionItems("list")
	...
	for i = 1, shown do
		...
		-- Create compact hash string from key fields
		hashData[i] = format("%d:%d:%d:%d:%d:%s", count or 0, minBid or 0, minInc or 0, buyout or 0, bid or 0, seller or "")
	end
	return table.concat(hashData, "|")
end
...
function IsDuplicatePage()
	...
	local currentHash = GetPageHash()
	local isDupe = (currentHash == private.lastPageHash)
	
	if not isDupe then
		private.lastPageHash = currentHash
	end
...
-- OPTIMIZED: LRU cache management to prevent memory bloat
local function AddToCacheLRU(itemString, cacheData)
	-- Add timestamp for auto-cleanup
	cacheData.timestamp = time()
	
	scanCache[itemString] = cacheData
	
	-- Update LRU tracking
	for i = #cacheAccessOrder, 1, -1 do
		if cacheAccessOrder[i] == itemString then
			tremove(cacheAccessOrder, i)
		end
	end
	tinsert(cacheAccessOrder, itemString)
	
	-- Enforce cache size limit (remove oldest entries)
	while #cacheAccessOrder > MAX_CACHE_SIZE do
		local oldest = tremove(cacheAccessOrder, 1)
		scanCache[oldest] = nil
	end
end

-- OPTIMIZED: Auto-cleanup old cache entries (>5 minutes old)
local function CleanOldCache()
	local now = time()
	local threshold = 300  -- 5 minutes
	
	for i = #cacheAccessOrder, 1, -1 do
		local itemString = cacheAccessOrder[i]
		local data = scanCache[itemString]
		if data and (now - (data.timestamp or 0) > threshold) then
			scanCache[itemString] = nil
			tremove(cacheAccessOrder, i)
		end
	end
end
...
function private:StopScanning()
	...
	if private.cache then
		...
		for itemString in pairs(private.cache.items) do
			AddToCacheLRU(itemString, private.cache)  -- OPTIMIZED: Use LRU cache management
		end
